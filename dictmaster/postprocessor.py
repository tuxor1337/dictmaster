...
class Processor(CancelableThread):
    ...
    def run(self):
        for f in sorted(os.listdir(self.input_directory)):
            ...
            if self._canceled:
                break
            self.process(os.path.join(self.input_directory, f))
...
class DictfileProcessor(Processor):
    ...
    def do_line(self, line):
        ...
        if self.subsubfieldSplit != None:
            syns = term.split(self.subsubfieldSplit)
            term = syns[0].strip()
        definition = ""
...
class HtmlProcessor(Processor):
    ...
    def do_html(self, doc):
        self.append(doc("dt").eq(0), doc("dd").eq(0))
    ...
    def do_html_term(self, dt):
        return ""

    def do_html_alts(self, dd, term):
        return []

    def do_html_definition(self, dd, term):
        return ""
